...
const UploadFilePageAdmin = () => {
    ...
    const handleSelectPayer = async(payer) => {
        ...
        try {
            ...
            const response = await creditFacilityService.getCreditFacilityDetail(payload);

            console.log("Datos de la facilidad de crédito:", response);
            const percentage = response.data.availablePercentage;
            
            setCreditLineLevel(percentage);
    ...
    return (
        /* Contenedor principal de toda la página en formato columna */
        <div className="flex flex-col min-h-screen gap-4">

            {/* 1. BANNER SUPERIOR: Límite de Crédito (Aparece arriba de todo) */}
            {creditLineLevel >= 90 && (
            ...
            <div className="flex flex-col md:flex-row flex-1 gap-4">
                                        ...
                                        <div className="md:flex md:w-full flex items-center space-x-3">
                                          ...
                                          ${selectedPayerId === payer.id ? 'bg-red-500' : 'bg-red-200'}`}>
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <h3 className={`font-medium break-words ${selectedPayerId === payer.id ? 'text-red-700' : 'text-gray-800'}`}>
                                                    {payer.name}
                                                </h3>
                ...
                {/* --- PANEL DERECHO (Centro de carga) --- */}
                <div className="flex-1 font-montserrat flex flex-col min-h-[70vh] md:min-h-0">
                    <div className="bg-white rounded-2xl shadow-lg flex-1 flex flex-col justify-between md:justify-evenly border-2 border-gray-200">

                        <div className="w-full max-w-2xl mx-auto px-4 md:px-0 py-8 md:py-0 mt-12">
                            <div className="text-center mb-8">
                                <h1 className="text-2xl md:text-3xl font-bold text-gray-800">Centro de carga de datos</h1>
                                <p className="text-sm md:text-base text-gray-600 mt-2">Gestión de archivos para proceso de confirming</p>
                            </div>
                            ...
                            <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 hover:border-red-300 transition duration-200">
                                ...
                                <div className="flex w-full gap-4">
                                    <div className="flex justify-center mt-6 w-full">
                                        ...
                                        <label
                                            htmlFor="file-upload"
                                            className={`px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 ${isUploading || creditLineLevel >= 90 ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'
                                                }`}
                                        >
                                ...
                                {selectedFile && (
                                    <div className="mt-4 p-3 bg-gray-100 rounded-md">
                                        <div className="flex items-center justify-between text-sm flex-wrap gap-2">
                                            <span className="font-medium text-gray-900 truncate max-w-[70%]">{selectedFile.name}</span>
                                ...
                                <div className="flex justify-center">
                                    <button
                                        className={`cursor-pointer w-full mt-4 rounded-2xl bg-red-600 px-4 py-1 text-sm font-medium text-white transition-colors duration-200 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 flex items-center justify-center ${creditLineLevel >= 90 || selectedFile == null ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'
                                            }`}
                                ...
                                <div className="mt-2 w-full">
                                    <button
                                        ...
                                        className={`flex justify-center items-center cursor-pointer w-full py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-red-700 hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 ${(!isAccepted || isUploading) ? 'opacity-50 cursor-not-allowed' : ''
                                            }`}
                                    >
                                        {isUploading ? (
                                            <>
                        ...
                        <div className="bg-gray-50 px-4 md:px-8 py-4 border-t border-gray-200 mt-2 rounded-b-2xl">
