...
const ApproveDocuments = () => {
    ...
    const handleApproveAll = async () => {
        ...
        const result = await SweetAlert.fire({
            ...
            preConfirm: async () => {
                try {
                    // 1. Obtener todos los documentos seleccionados
                    const selectedDocs = getSelectedDocuments().filter(doc =>
                        selected.includes(doc.document_id)
                    );

                    // 2. Agrupar los documentos por 'agreementId' en un formato de array
                    const groupedDocs = selectedDocs.reduce((acc, doc) => {
                        let group = acc.find(g => g.agreementId === doc.agreementId);
                        if (group) {
                            group.documentIds.push(doc.document_id);
                        } else {
                            acc.push({
                    ...
                    // 3. Crear el payload final para la única llamada a la API
                    const bulkPayload = {
                        approvals   : groupedDocs,
                    ...
                    // 4. Llamar al nuevo método del servicio que hace la llamada única
                    //    (Este método debe ser creado en 'agreementService')
                    await agreementService.approveDocumentsInBulk(bulkPayload);

                } catch (error) {
                    SweetAlert.showValidationMessage(`Error: ${error.message || 'Ocurrió un error al aprobar.'}`);
