...
public class SsoService {
    ...
    public HandoffDTOResponse processHandoff(String appCode, String otc, String bearerTokenRequest) {
        ...
        try {
            ...
            log.info("url para petición: {}", payDaviviendaUrl);
            ...
            headers.set("X-Authorization", "Bearer " + payBearerToken);
            headers.set("Accept", "application/json");

            String jsonBody = "{\"otc\":\"" + otc + "\"}";
            log.info("[DEBUG] JSON Body exacto: {}", jsonBody);
            log.info("[DEBUG] Content-Length: {} bytes", jsonBody.getBytes(java.nio.charset.StandardCharsets.UTF_8).length);
