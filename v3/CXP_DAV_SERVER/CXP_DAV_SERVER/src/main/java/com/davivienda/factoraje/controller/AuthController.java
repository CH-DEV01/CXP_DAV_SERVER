...
public class AuthController {
    ...
    public ResponseEntity<Boolean> validateSession(@CookieValue(name = "token", required = false) String token) {
          log.info("[Validate-session endpoint] - Validating session with token: {}", token);
    ...
    public ResponseEntity<String> login(
        ...
        authService.login(request.getDui(), response);

        return ResponseEntity.ok("Inicio de sesión exitoso. Cookie configurada.");
    }
