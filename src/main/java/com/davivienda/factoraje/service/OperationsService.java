...
public class OperationsService {
    ...
    public CalculateDTOResponse calculate(List<CalculateDTORequest> request) {
        ...
        for (CalculateDTORequest dto : request) {
            ...
            BigDecimal discount_rate = percentage.multiply(interestRate);
            log.info("Tasa de descuento: {}", discount_rate);

            System.out.println(dto.getDiffDays());

            BigDecimal amount = dto.getAmount();
            BigDecimal amountToFinance = amount.multiply(percentage);
            log.info("Factor de descuento: {}", percentage);

            // Vamos a refactorizar el calculo de intereses para usar el monto a financiar en lugar del monto neto de la factura
